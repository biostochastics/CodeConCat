...
@pytest.mark.skip(reason="Repository tests are time-consuming and network-dependent. Security features are already well-tested in unit tests.")
class TestRepositoryValidation:
        ...
        try:
            ...
            try:
                ...
                if repo['name'] == 'flask-microblog':
                    ...
                    logger.error(f"Detailed traceback for flask-microblog written to {debug_log_path}")
                    print(f"--- START content of {debug_log_path} ---")
                    with open(debug_log_path, "r") as f_read:
                        print(f_read.read())
                    print(f"--- END content of {debug_log_path} ---")
                logger.error(f"Error running CodeConCat on {repo['name']}: {e}") # General error log for all repos
                raise
    ...
    def test_repository_semgrep_validation(self, repo, tmp_path):
        ...
        with patch('codeconcat.validation.semgrep_validator.semgrep_validator.is_available', return_value=False):
        ...
        try:
            ...
            with patch('codeconcat.validation.semgrep_validator.SemgrepValidator.scan_file', return_value=mock_findings):
                with patch('codeconcat.validation.semgrep_validator.semgrep_validator.is_available', return_value=True):
                    ...
                    try:
                        ...
                        if repo["expected_findings"] > 0:
                            assert "WARNING" in output or "security" in output.lower(), f"Expected security findings in {repo['name']} output"
        ...
        try:
            ...
            try:
                ...
                if repo["expected_findings"] > 0:
                    ...
                    logger.info(f"CodeConCat correctly failed in strict mode for {repo['name']}: {excinfo.value}")
                    assert "suspicious content" in str(excinfo.value).lower() or "security" in str(excinfo.value).lower()
                else:
                    # Clean repositories should pass even in strict mode
                    output = run_codeconcat(config)
                    logger.info(f"CodeConCat completed in strict mode for clean repo {repo['name']}")
                    assert output, "No output was generated"
                    # No need to check file existence since run_codeconcat returns string
