...
""")
    ...
    def mock_semgrep(self):
        ...
        with patch('codeconcat.validation.semgrep_validator.semgrep_validator.is_available', return_value=True):
            with patch('codeconcat.validation.semgrep_validator.SemgrepValidator.scan_file') as mock_scan:
    ...
    @pytest.mark.parametrize("scenario", SECURITY_SCENARIOS, ids=[s["name"] for s in SECURITY_SCENARIOS])
    def test_security_scenarios(self, scenario, sample_project, mock_semgrep, tmp_path):
        ...
        logger.debug(f"Files in sample project: {list(sample_project.rglob('*'))[:10]}")  # Show first 10 files
    ...
    severity: WARNING
        ...
        if scenario["should_pass"]:
            try:
                ...
                with patch('codeconcat.validation.setup_semgrep.install_semgrep', return_value=True):
                    with patch('codeconcat.validation.setup_semgrep.install_apiiro_ruleset', return_value="/mock/ruleset/path"):
                ...
                assert output_path.exists(), f"Output file not created for scenario {scenario['name']}"
                ...
                if scenario["config"].get("enable_semgrep", False):
                    ...
                    suspicious_files = [f for f in output_data["files"] if "suspicious.py" in f["file_path"] or "app.js" in f["file_path"]]
                    assert len(suspicious_files) > 0, "Expected suspicious files not found in output"
            ...
            assert "security" in str(excinfo.value).lower() or "suspicious" in str(excinfo.value).lower(), \
                f"Expected security-related exception for scenario {scenario['name']}"
    
    @pytest.mark.skip(reason="CLI test needs refactoring - security features are already tested in other tests")
...
rules:
  - id: apiiro.command-injection
    ...
    severity: HIGH
            ...
            with patch('codeconcat.validation.semgrep_validator.SemgrepValidator.scan_file') as mock_scan:
                with patch('codeconcat.validation.setup_semgrep.install_apiiro_ruleset', return_value=str(ruleset_dir)):
                    ...
                    with patch('codeconcat.validation.semgrep_validator.semgrep_validator.is_available', return_value=True):
                        ...
                        assert semgrep_validator.ruleset_path == str(ruleset_dir), "Ruleset not properly set"
                        ...
                        assert len(findings) > 0, "No security findings detected"
                        apiiro_findings = [f for f in findings if f.get('rule_id') == 'apiiro.command-injection']
