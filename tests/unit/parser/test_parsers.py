...
class TestParsers:
        ...
        if use_enhanced:
            ...
            if parser_class_name:
                ...
                try:
                    ...
                    module_name = f"codeconcat.parser.language_parsers.enhanced_{normalized_language}_parser"
    ...
    def _validate_parse_result(self, 
        ...
        if "declarations" in file_expected:
            ...
            if missing:
                errors.append(
                    f"Missing declarations in {basename}: {missing}"
                )
                
            if extra:
                errors.append(
                    f"Extra declarations in {basename}: {extra}"
                )
        ...
        if "imports" in file_expected:
            ...
            if missing:
                errors.append(
                    f"Missing imports in {basename}: {missing}"
                )
                
            if extra:
                errors.append(
                    f"Extra imports in {basename}: {extra}"
                )
    ...
    def _generate_expected_output(self, 
        ...
        expected = {
            ...
            "declarations_with_docstrings": [d.name for d in parse_result.declarations if d.docstring]
        }
    ...
    def test_language_parser(self, config: CodeConCatConfig, corpus_dir: str, language: str):
        ...
        for file_path in files:
            ...
            if generate_expected:
                generated_expected.update(
                    self._generate_expected_output(result, file_path)
                )
                continue
