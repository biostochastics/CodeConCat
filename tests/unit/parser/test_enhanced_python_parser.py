...
'''
    ...
    def test_python_parser_parse(self, python_code_sample):
        ...
        print(
            f"Found calculate_stats function at lines {calculate_stats_func.start_line}-{calculate_stats_func.end_line}"
        )
        ...
        if person_class.children:
            ...
            set_address_method = next(
                (m for m in person_class.children if m.name == "set_address"), None
            )
            ...
            if set_address_method.children:
                print(
                    f"Found {len(set_address_method.children)} nested functions in set_address method:"
                )
                ...
                if log_change_func:
                    print(
                        f"Found nested log_change function at lines {log_change_func.start_line}-{log_change_func.end_line}"
                    )
    ...
    def test_python_parser_nested_declarations(self, python_code_sample):
        ...
        print(
            f"Found {len(declarations_with_children)} declarations with children (nested declarations)"
        )
        ...
        person_class = next(
            (d for d in result.declarations if d.kind == "class" and d.name == "Person"), None
        )
        assert person_class is not None, "Person class not found"
