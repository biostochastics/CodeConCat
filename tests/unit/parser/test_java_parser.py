    ...
    def test_java_parser_parse(self, java_code_sample):
        ...
        if test_class:
            ...
            if test_class.children:
                ...
                get_value_method = next((m for m in test_class.children if m.name == "getValue"), None)
                if get_value_method:
                    print(f"Found getValue method at lines {get_value_method.start_line}-{get_value_method.end_line}")
                    if get_value_method.docstring and "get the value" in get_value_method.docstring.lower():
                        print("Method docstring correctly detected")
                
                # Try to find nested class
                nested_class = next((m for m in test_class.children if m.name == "NestedClass"), None)
                if nested_class:
                    print(f"Found NestedClass at lines {nested_class.start_line}-{nested_class.end_line}")
