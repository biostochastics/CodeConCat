...
class TestMarkdownWriterSimple:
    ...
    def test_write_markdown_with_ai_preamble(self):
        ...
        file_data.render_markdown_chunks = Mock(return_value=["### /test/sample.py", "```python", "pass", "```"])
        
        items = [file_data]
        
        with patch('codeconcat.writer.markdown_writer.print_quote_with_ascii'):
            with patch('codeconcat.writer.markdown_writer.generate_ai_preamble', return_value="AI Preamble"):
                result = write_markdown(items, config)
