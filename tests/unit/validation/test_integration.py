...
class TestValidationIntegration:
    ...
    def test_verify_file_signatures(self, tmp_path):
        ...
        files = [
            ...
            ParsedFileData(file_path=str(binary_file), content=b'\x00\x01\x02\x03', language="binary"),
        ]
        
        # Verify file signatures
        with patch('codeconcat.validation.security.SecurityValidator.is_binary_file') as mock_is_binary:
    ...
    def test_verify_file_signatures_invalid(self, tmp_path):
        ...
        # Create ParsedFileData objects
        files = [
            ParsedFileData(file_path=str(binary_py_file), content=b'\x00\x01\x02\x03', language="python"),
        ]
        
        # Verify file signatures - should raise ValidationError
        with patch('codeconcat.validation.security.SecurityValidator.is_binary_file', return_value=True):
