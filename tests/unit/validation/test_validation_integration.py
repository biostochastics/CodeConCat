...
class TestValidationIntegration:
    ...
    def test_validate_input_files(self, tmp_path):
        ...
        files_to_process = [
            ParsedFileData(
                file_path=str(file1), content="def add(a, b): return a + b", language="python"
            ),
            ParsedFileData(
                file_path=str(file2), content="def subtract(a, b): return a - b", language="python"
            ),
        ]
    ...
    def test_validate_config_values(self):
        ...
        # Valid configuration
        valid_config = CodeConCatConfig(format="markdown", output="output.md")
    ...
    def test_setup_semgrep_already_available(
        self, mock_install_ruleset, mock_install_semgrep, mock_validator
    ):
    ...
    @patch("codeconcat.validation.setup_semgrep.install_apiiro_ruleset")
    @patch("shutil.which")
    def test_setup_semgrep_install(
        self, mock_which, mock_install_ruleset, mock_install_semgrep, mock_validator
    ):
    ...
    def test_verify_file_signatures(self, tmp_path):
        ...
        with patch(
            ...
            side_effect=lambda path: "binary" in str(path),
        ):
            ...
            files_to_process = [
                ...
                ParsedFileData(
                    file_path=str(tmp_path / "binary.bin"),
                    content="binary data",
                    language="unknown",
                ),
            ]
