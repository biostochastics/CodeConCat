...
def fetch_repo_files(repo_name: str, config: CodeConCatConfig) -> Tuple[List[ParsedFileData], str]:
        ...
        if os.path.exists(temp_dir):
            ...
            for root, _, filenames in os.walk(temp_dir):
                ...
                for filename in filenames:
                    ...
                    if filename.endswith('.ts') and not filename.endswith('.d.ts') and not filename.endswith('.tsx'):
            ...
            logger.info(f"Found {len(ts_files)} TypeScript files out of {len(all_files)} total files")
            if ts_files:
            ...
            if (len(files) < 10 and len(ts_files) > 10) or js_files or tsx_files:
                logger.warning(f"Collector only found {len(files)} files but found {len(ts_files)} TS, {len(js_files)} JS, and {len(tsx_files)} TSX files")
                ...
                for file_path in ts_files[:50]:  # Process up to 50 files per type
                    try:
                        with open(file_path, 'r', encoding='utf-8') as f:
                            content = f.read()
                        direct_files.append(ParsedFileData(
                            file_path=file_path,
                            language="typescript",
                            content=content
                        ))
                ...
                if direct_files:
                    logger.info(f"Successfully created {len(direct_files)} ParsedFileData objects directly")
    ...
    try:
        ...
        if files and len(files) > 0:
            ...
            logger.warning(f"No files found in cloned repository via API. Will try local fallback.")
    except Exception as e:
    ...
    for local_path in potential_paths:
        if os.path.exists(local_path) and os.path.isdir(local_path):
            ...
            if files:
                logger.info(f"First 5 files found:")
                ...
                temp_dir = tempfile.mkdtemp(prefix=f"codeconcat_test_{repo_name.replace('/', '_')}_local_")
                return files, temp_dir
...
def repomix_repo():
    ...
    config = CodeConCatConfig(
        ...
        include_paths=[
            ...
            "src/**/*", "website/**/*", "tests/**/*", "core/**/*",
            # Look for config files in root
            "*.js", "*.ts", "*.json", "*.config.ts"
        ],
    ...
    if len(files) <= 1:
        logger.warning(f"Only found {len(files)} files through collector. Searching directory directly.")
        if os.path.exists(temp_dir):
            ...
            if len(dir_contents) < 5:
                ...
                full_temp_dir = tempfile.mkdtemp(prefix=f"repomix_full_")
            ...
            if ts_count > 0:
                logger.info(f"First TypeScript file: {next((f for f in all_files if f.endswith('.ts') and not f.endswith('.d.ts')), 'None')}")
                ...
                for filename in filenames:
                    if filename.endswith('.ts') and not filename.endswith('.d.ts'):
                        ...
                        try:
                            ...
                            ts_files.append(ParsedFileData(
                                file_path=file_path,
                                language="typescript",
                                content=content
                            ))
    ...
    try:
        ...
        if files:
            ...
            ts_files = [f for f in files if f.file_path.endswith('.ts') and not f.file_path.endswith('.tsx')]
            if ts_files:
    ...
    for file_data in python_files:
        ...
        if hasattr(file_data, 'declarations') and file_data.declarations:
            ...
            if class_count > 0 or func_count > 0 or method_count > 0:
                logger.debug(f"Found {class_count} classes, {func_count} functions, {method_count} methods in {file_data.file_path}")
            else:
...
def test_repomix_typescript_parsers(repomix_repo):
    ...
    typescript_files = [f for f in files if f.file_path.endswith('.ts') and not f.file_path.endswith('.tsx')]
    ...
    for file_data in files_to_process:
        ...
        try:
            ...
            # Log parse status
            logger.info(f"Parsed {file_data.file_path}: {len(declarations)} declarations, {len(imports)} imports")
    ...
    # Report summary
    logger.info(f"TypeScript Test Summary:")
    ...
    success_rate = (processed_files - len(failed_files)) / processed_files if processed_files > 0 else 0
    if success_rate < 0.5:
        logger.warning(f"Low success rate for TypeScript parsing: {success_rate:.2f} (failed {len(failed_files)}/{processed_files})")
    ...
    if declaration_count == 0 and import_count == 0:
        ...
        try:
            ...
            logger.info(f"Fallback TypeScript example processed successfully")
            if hasattr(result, 'declarations'):
            ...
            assert hasattr(result, 'declarations') or hasattr(result, 'imports'), "Parser failed to extract from example"
        except Exception as e:
    ...
    print(f"TypeScript test complete: Processed {processed_files} files with {declaration_count} declarations")
...
def test_repomix_javascript_parsers(repomix_repo):
    ...
    for file_data in files:
        if file_data.language.lower() == "javascript":
            try:
                ...
                total_files += 1
                total_declarations += len(result.declarations)
                logger.debug(f"Parsed {file_data.file_path} with {len(result.declarations)} declarations")
            except Exception as e:
    ...
    assert len(found_kinds) > 0, f"Found no expected declaration kinds, only {declaration_kinds.keys()}"
...
def test_tsx_react_component_parsing(repomix_repo):
    ...
    for file_data in tsx_files:
        ...
        print(f"Found {len(component_declarations)} potential React components in {file_data.file_path}")
