...
def install_semgrep():
    ...
    Returns:
        str: Path to the installed semgrep executable
    ...
    try:
        ...
        if not semgrep_path:
            raise ValidationError("Semgrep installed but executable not found in PATH")
        return semgrep_path
    except subprocess.CalledProcessError as e:
        logger.error(f"Failed to install semgrep: {e.stderr}")
        raise ValidationError(f"Failed to install semgrep: {e.stderr}")
    except Exception as e:
        logger.error(f"Error installing semgrep: {e}")
        raise ValidationError(f"Failed to install semgrep: {e}")
