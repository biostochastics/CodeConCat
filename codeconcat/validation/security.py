...
FILE_SIGNATURES = {
    ...
    b'\x50\x4b\x03\x04\x14\x00\x06\x00': {'type': 'document', 'description': 'MS Office document (OOXML)'},
    # Images
...
class SecurityValidator:
    ...
    @staticmethod
    def compute_file_hash(file_path: Union[str, Path], algorithm: str = 'sha256', use_cache: bool = True) -> str:
        """
        ...
        for name, pattern in patterns.items():
            if pattern.search(sanitized):
                ...
                if name == "secrets_pattern":
                    ...
                    sanitized = pattern.sub(
                        r"/* POTENTIALLY DANGEROUS CONTENT REMOVED: \g<0> */", 
                        sanitized
                    )
    ...
    @staticmethod
    def check_for_suspicious_content(file_path: Union[str, Path], use_semgrep: bool = False) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if pattern_findings:
                logger.warning(f"Suspicious content detected in {path}: {', '.join(pattern_findings)}")
            ...
            raise ValidationError(f"Failed to check file for suspicious content: {path}", 
    ...
    def is_binary_file(file_path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            text_extensions = {
                ...
                '.yaml', '.yml', '.toml', '.ini', '.c', '.cpp', '.h', '.hpp', '.java',
                '.cs', '.go', '.rb', '.php', '.rs', '.sh', '.bat', '.ps1'
            }
    ...
    @staticmethod
    def generate_integrity_manifest(directory: Union[str, Path], recursive: bool = True) -> Dict[str, str]:
        """
        ...
        try:
            ...
            if isinstance(e, ValidationError):
                raise
            raise ValidationError(f"Failed to generate integrity manifest for {directory}", 
                                original_exception=e)
