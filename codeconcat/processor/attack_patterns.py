...
C_PATTERNS = [
    ...
    SecurityPattern(
        ...
        pattern=re.compile(
            r'\b(system|popen|execl|execle|execlp|execv|execvp)\s*\([^"]+\)', re.IGNORECASE
        ),
        severity=Severity.CRITICAL,
...
PYTHON_PATTERNS = [
    SecurityPattern(
        ...
        pattern=re.compile(
            r'\beval\s*\([^"\']*(?:user_input|request\.|input\(|argv|f["\']|%|\+)[^)]*\)',
            re.IGNORECASE,
        ),
        severity=Severity.CRITICAL,
    ...
    SecurityPattern(
        ...
        pattern=re.compile(
            r'\bexec\s*\([^"\']*(?:user_|input|another_|request\.|argv|f["\']|%|\+)[^)]*\)',
            re.IGNORECASE,
        ),
        severity=Severity.CRITICAL,
    ...
    SecurityPattern(
        ...
        pattern=re.compile(
            r'(execute|executemany|query)\s*\([^)]*["\'].*?["\'].*?[+%].*?(user_|request\.|input\(|argv|\$|f["\'])',
    ...
    SecurityPattern(
        ...
        pattern=re.compile(
            r'(os\.system|subprocess\.(call|Popen|run))\s*\([^)]*(?:user_|cmd|command|input|request\.|argv|[+%]|f["\'])',
            re.IGNORECASE,
        ),
        severity=Severity.CRITICAL,
...
R_PATTERNS = [
    ...
    SecurityPattern(
        ...
        pattern=re.compile(
            r"dbGetQuery\s*\([^,)]+,\s*[^,)]+(?:paste|sprintf|paste0)\s*\(",
            re.IGNORECASE | re.DOTALL,
        ),
        severity=Severity.CRITICAL,
        message="Potential SQL injection via string concatenation",
...
PHP_PATTERNS = [
    SecurityPattern(
        ...
        pattern=re.compile(
            r'(mysql_query|mysqli_query|pg_query|PDO::query)\s*\([^)]*["\'].*\s*\.\s*\$\w+',
            re.IGNORECASE,
        ),
        severity=Severity.CRITICAL,
    ...
    SecurityPattern(
        ...
        pattern=re.compile(
            r"(system|exec|shell_exec|passthru|eval)\s*\([^)]*\$_(GET|POST|REQUEST)", re.IGNORECASE
        ),
        severity=Severity.CRITICAL,
    ...
    SecurityPattern(
        ...
        pattern=re.compile(
            r"(include|require|include_once|require_once)\s*\([^)]*\$_(GET|POST|REQUEST)",
            re.IGNORECASE,
        ),
        severity=Severity.CRITICAL,
...
JS_TS_PATTERNS = [
    ...
    SecurityPattern(
        ...
        pattern=re.compile(
            r'(\[["\'`]?(__proto__|prototype|constructor)["\'`]?\]|\.__proto__|\.(prototype|constructor))\s*=',
            re.IGNORECASE,
        ),
        severity=Severity.HIGH,
...
CROSS_LANGUAGE_PATTERNS = [
    SecurityPattern(
        ...
        pattern=re.compile(
            r'(secret[_-]?key|api[_-]?key|private[_-]?key)\s*[:=]\s*["\'][^"\']{10,}["\']',
            re.IGNORECASE,
        ),
        severity=Severity.HIGH,
    ...
    SecurityPattern(
        ...
        pattern=re.compile(
            r"(bc1|[13])[a-zA-HJ-NP-Z0-9]{25,62}|0x[a-fA-F0-9]{40}|4[0-9AB][1-9A-HJ-NP-Za-km-z]{93}"
        ),
        severity=Severity.MEDIUM,
