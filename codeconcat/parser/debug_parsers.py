...
from codeconcat.base_types import Declaration, ParseResult, CodeConCatConfig
...
def compare_parsers_for_file(file_path: str) -> ParserComparisonResult:
    ...
    try:
        ...
        if enhanced_parser:
            ...
            result.enhanced_metrics = compute_declaration_metrics(result.enhanced_result.declarations)
    except Exception as e:
    ...
    try:
        ...
        if tree_sitter_parser:
            ...
            result.tree_sitter_metrics = compute_declaration_metrics(result.tree_sitter_result.declarations)
    except Exception as e:
...
def analyze_parser_differences(result: ParserComparisonResult) -> ParserComparisonResult:
    ...
    if result.basic_result and result.enhanced_result:
        ...
        if new_imports:
            result.import_diffs.append(
                f"New imports found in enhanced: {new_imports}"
            )
