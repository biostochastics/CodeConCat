        ...
        if self.block_comment_start and self.block_comment_end:
            ...
            for i in range(start, min(end + 1, len(lines))):
                ...
                if not in_comment and line.startswith(self.block_comment_start):
                    ...
                    in_comment = True
                    if line.endswith(self.block_comment_end) and len(line) > len(self.block_comment_start) + len(self.block_comment_end):
                        # Single-line comment
                        return line[len(self.block_comment_start):-len(self.block_comment_end)].strip()
