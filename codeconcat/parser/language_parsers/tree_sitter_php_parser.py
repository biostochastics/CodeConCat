...
class TreeSitterPhpParser(BaseTreeSitterParser):
    ...
    def _run_queries(self, root_node: Node, byte_content: bytes) -> tuple[List[Declaration], List[str]]:
                ...
                if query_name == "imports":
                    for node, capture_name in captures:
                        if capture_name == "import_path":
                            ...
                            if node.parent and node.parent.type == 'namespace_use_clause':
                                ...
                                if group_node:
                                    prefix_node = next((n for n, name in query.captures(group_node) if name == 'import_path'), None)
                                    if prefix_node:
                                        prefix = byte_content[prefix_node.start_byte:prefix_node.end_byte].decode("utf8", errors="ignore")
                    ...
                    for node, capture_name in captures:
                        ...
                        while decl_node.parent and decl_node.type not in [
                            ...
                            'trait_declaration', 'function_definition', 'const_declaration',
                            'class_const_declaration', 'property_declaration', 'program'
                        ]:
                        ...
                        if capture_name in ["namespace", "class", "interface", "trait", "function", "constant", "class_constant", "property"]:
                    ...
                    for decl_id, data in node_capture_map.items():
                        ...
                        elif kind == 'anonymous_class_declaration': kind = 'anonymous_class'
                        elif kind == 'interface_declaration': kind = 'interface'
                        elif kind == 'trait_declaration': kind = 'trait'
                        elif kind == 'function_definition': kind = 'function'
                        elif kind == 'method_declaration': kind = 'method'
                        elif kind == 'constructor_with_promotion': kind = 'constructor'
                        elif kind == 'const_declaration': kind = 'constant'
                        elif kind == 'class_const_declaration': kind = 'class_constant'
                        elif kind == 'property_declaration': kind = 'property'
                        elif kind == 'namespace_definition': kind = 'namespace'
                        elif kind == 'enum_declaration': kind = 'enum'
                        elif kind == 'global_declaration': kind = 'global_variable'
                        # Keep existing kind if already mapped

                        if kind not in ["namespace", "class", "anonymous_class", "interface", "trait", "function", "method", "constructor", "constant", "class_constant", "property", "enum", "global_variable"]:
                        ...
                        name_text = byte_content[name_node.start_byte:name_node.end_byte].decode("utf8", errors="ignore") if name_node else "<anonymous>"
                        ...
                        modifier_nodes = [(n, cname) for n, cname in node_captures 
                                          if cname in ["modifiers", "class_modifier", "property_modifiers", 
                                                      "const_modifiers", "constructor_modifiers", "prop_promotion_modifiers"]]
                        
                        for mod_node, _ in modifier_nodes:
                            mod_text = byte_content[mod_node.start_byte:mod_node.end_byte].decode("utf8", errors="ignore")
                            modifiers.add(mod_text)
                            
                        # Check for attributes (PHP 8+)
                        attr_nodes = [(n, cname) for n, cname in node_captures 
                                     if cname in ["class_attributes", "method_attributes", "function_attributes", 
                                                 "interface_attributes", "trait_attributes", "property_attributes", 
                                                 "enum_attributes"]]
                        ...
                        return_type_node = next((n for n, cname in node_captures if cname == "return_type"), None)
                        ...
                        full_name = f"{current_namespace}\\{name_text}" if current_namespace and name_text != "<anonymous>" else name_text
                        ...
                        if decl_id not in declaration_map:
                            ...
                            if declaration_map[decl_id]['name'] == "<anonymous>" and full_name != "<anonymous>":
        ...
        logger.debug(f"Tree-sitter PHP extracted {len(declarations)} declarations and {len(sorted_imports)} imports.")
