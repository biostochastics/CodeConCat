...
class JavaParser(BaseParser):
    ...
    def _setup_patterns(self):
        ...
        # Interface pattern
        interface_pattern = (
            r"^\s*(?:public\s+|private\s+|protected\s+)*interface\s+(?P<n>[a-zA-Z_][a-zA-Z0-9_]*)"
        )
    ...
    def parse(self, content: str) -> List[Declaration]:
                                    ...
                                    for nested_kind, nested_pattern in self.patterns.items():
                                        ...
                                        if nested_match:
                                            ...
                                            if nested_name:
                                                ...
                                                while (
                                                    m < len(nested_words)
                                                    and nested_words[m] in self.modifiers
                                                ):
        ...
        top_level_declarations = [
            d for d in declarations if d.kind in ("class", "interface", "enum")
        ]
