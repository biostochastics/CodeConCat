        ...
        while i <= end and line_counter < max_line_iterations:
            ...
            for kind, pattern in self.patterns.items():
                ...
                if match:
                    ...
                    block_found = False
                    requires_block_end_find = kind in ["function", "nested_function", "method", "struct", "enum", "trait", "impl", "macro", "closure"]
                        ...
                        if has_block_opener:
                            ...
                            try:
                                ...
                                if calculated_end_idx != -1:
                                    ...
                                    end_line = end_line_idx + 1 # Convert 0-indexed to 1-indexed for Declaration
                                else:
                                    # Log error if block end not found and estimate
                                    logger.warning(f"Could not find matching }} for {{ at line {i+1}. Declaration: {kind} {name}")
                                    end_line_idx = min(i + 49, len(lines) - 1) # Estimate index
                                    end_line = end_line_idx + 1
                            except Exception as e:
                                logger.error(f"Error finding block end for {kind} {name} at line {i+1}: {e}", exc_info=True)
                    ...
                    if not requires_block_end_find or (requires_block_end_find and not block_found and ';' not in line):
                        end_line_idx = i
                        end_line = i + 1

                    # Safety check for end_line (ensure it's at least start_line)
                    if end_line < start_line:
                         logger.warning(f"Corrected invalid end_line {end_line} for {kind} {name} at L{start_line}, setting end to {start_line}")
                         end_line = start_line
                    ...
                    if block_found and end_line_idx > i:
                        ...
                        nested_end_idx = end_line_idx - 1  # Line index before the closing brace line
                        
                        # Ensure the calculated range for nested processing is valid
                        if nested_start_idx <= nested_end_idx:
                            logger.debug(f"Processing nested declarations within {kind} {name} (lines {nested_start_idx+1}-{nested_end_idx+1})")
                            ...
                            logger.debug(f"Nested processing completed for {kind} {name} at line {end_line}")
                        else:
                            logger.debug(f"Skipping nested processing for {kind} {name} due to invalid range ({nested_start_idx+1}-{nested_end_idx+1})")
            ...
            while i < max_end_line and nesting_level > 0 and iterations < max_iterations:  # Only continue while nesting level > 0
                ...
                while col < len(line):
                    ...
                    # Handle nesting, but only if not in a string or character literal
                        ...
                        if char == open_char:
                            nesting_level += 1
                            logger.debug(f"L{i+1}:{col+1} Found '{open_char}', nesting level -> {nesting_level}")
                        elif char == close_char:
                            nesting_level -= 1
                            logger.debug(f"L{i+1}:{col+1} Found '{close_char}', nesting level -> {nesting_level}")
                            if nesting_level == 0:
                                logger.debug(f"L{i+1}:{col+1} Found final '{close_char}', returning line {i+1}")
                                return i
