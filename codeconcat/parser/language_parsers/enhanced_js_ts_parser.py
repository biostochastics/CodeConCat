...
class EnhancedJSTypeScriptParser(EnhancedBaseParser):
    ...
    def _setup_js_ts_patterns(self):
        ...
        self.patterns["type"] = re.compile(
            r"^\s*(?:export\s+)?type\s+(?P<name>[\w<>]+)\s*=", re.MULTILINE
        )

        # Variable declarations
        self.patterns["variable"] = re.compile(
            r"^\s*(?:export\s+)?(?:const|let|var)\s+(?P<name>\w+)\s*(?:\s*\w+)?\s*=", re.MULTILINE
        )

        # Method definition pattern (within classes)
        self.patterns["method"] = re.compile(
            r"^\s*(?:static\s+)?(?:async\s+)?(?:get\s+|set\s+)?(?P<name>[\w$]+)\s*\(", re.MULTILINE
        )

        # Object method/property pattern
        self.patterns["object_method"] = re.compile(
            r"^\s*(?P<name>[\w$]+)\s*:\s*function\s*\(", re.MULTILINE
        )

        # Anonymous function pattern - for nested function detection
        self.patterns["anon_function"] = re.compile(r"^\s*function\s*\(", re.MULTILINE)
            ...
            if declaration_match:
                ...
                if has_block:
                    ...
                    if brace_level <= 0:
                        ...
                        if (
                            normalized_kind in ["class", "function", "method", "interface"]
                    ...
                    while j <= end:
                        ...
                        if j_line.startswith("{"):
                            ...
                            if parent_declaration:
                                ...
                                declarations.append(declaration)
                                logger.debug(
                                    f"Added top-level declaration: {normalized_kind} {name}"
                                )
                    ...
                    if not has_block:
                        ...
                        if parent_declaration:
                            parent_declaration.children.append(declaration)
                            logger.debug(
                                f"Added nested declaration (no block) to parent {parent_declaration.name}: {normalized_kind} {name}"
                            )
                        else:
                            declarations.append(declaration)
