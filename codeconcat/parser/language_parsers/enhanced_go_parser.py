...
class EnhancedGoParser(EnhancedBaseParser):
    ...
    def _setup_standard_patterns(self):
        ...
        self.modifiers = {
            ...
            "package", "import", "map", "chan", "defer", "go", "select",
    ...
    def _setup_go_patterns(self):
        ...
        self.patterns["method"] = re.compile(
            r"^\s*func\s+\(\s*\w+\s+\*?\s*\w+\s*\)\s+(?P<name>\w+)\s*\(",
            re.MULTILINE
        )
        ...
        self.patterns["type"] = re.compile(
            r"^\s*type\s+(?P<name>\w+)\s+(?!struct|interface)\w+",
            re.MULTILINE
        )
        
        # Constant declarations
        self.patterns["const"] = re.compile(
            r"^\s*(?:const|var)\s+(?P<name>\w+)(?:\s+[\w\[\]{}*<>.]+)?\s*=",
            re.MULTILINE
        )
    ...
    def _process_imports(self, line: str, imports: List[str], i: int, lines: List[str]) -> Tuple[bool, int]:
        """
