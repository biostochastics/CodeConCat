...
def _clean_rust_doc_comment(comment_block: List[str]) -> str:
    ...
    is_inner_doc = comment_block[0].startswith(("//!", "/*!")) if comment_block else False # //! applies to parent
...
class TreeSitterRustParser(BaseTreeSitterParser):
    ...
    def _run_queries(self, root_node: Node, byte_content: bytes) -> tuple[List[Declaration], List[str]]:
            ...
            for node, _ in doc_captures:
                ...
                if is_block:
                    if current_doc_block:
                         doc_comment_map[last_comment_line] = {'text': current_doc_block, 'inner': current_block_inner}
                    doc_comment_map[current_end_line] = {'text': comment_text.splitlines(), 'inner': is_inner}
                    current_doc_block = []
                    last_comment_line = current_end_line
                else: # Line comment
                    if current_start_line == last_comment_line + 1 and is_inner == current_block_inner:
                ...
                if query_name == "imports":
                    for node, capture_name in captures:
                        if capture_name == "import_path":
                            import_path = byte_content[node.start_byte:node.end_byte].decode("utf8", errors="ignore")
                    ...
                    for node, capture_name in captures:
                        ...
                        while decl_node.parent and decl_node.type not in [
                            ...
                            'type_item', 'macro_definition', 'source_file'
                        ]:
                        ...
                        if capture_name in ["function", "struct", "enum", "trait", "impl", "module", "constant", "static", "type_alias", "macro_definition", "method", "assoc_type", "assoc_const"]:
                    ...
                    for decl_id, data in node_capture_map.items():
                        ...
                        kind_map = {
                            ...
                            'impl_item': 'impl', 'mod_item': 'module',
                            'const_item': 'constant', 'static_item': 'static',
                            'type_item': 'type_alias', 'macro_definition': 'macro',
                            'method': 'method', # Use specific capture name
                            'assoc_type': 'associated_type', 'assoc_const': 'associated_constant'
                        }
                        kind = kind_map.get(kind, kind) # Use mapped kind or original capture name

                        if kind not in ['function', 'struct', 'enum', 'trait', 'impl', 'module', 'constant', 'static', 'type_alias', 'macro', 'method', 'associated_type', 'associated_constant']:
                        ...
                        if kind == 'impl':
                            ...
                            trait_node = next((n for n, cname in node_captures if cname == "trait_name"), None)
                            type_node = next((n for n, cname in node_captures if cname == "impl_type"), None)
                            trait_name = byte_content[trait_node.start_byte:trait_node.end_byte].decode("utf8", errors="ignore") if trait_node else None
                            type_name = byte_content[type_node.start_byte:type_node.end_byte].decode("utf8", errors="ignore") if type_node else "<unknown>"
                            name = f"{trait_name} for {type_name}" if trait_name else type_name
                        elif kind == 'method':
                            name_node = next((n for n, cname in node_captures if cname == "method_name"), None)
                            if name_node: name = byte_content[name_node.start_byte:name_node.end_byte].decode("utf8", errors="ignore")
                        elif kind == 'associated_type':
                            name_node = next((n for n, cname in node_captures if cname == "assoc_type_name"), None)
                            if name_node: name = byte_content[name_node.start_byte:name_node.end_byte].decode("utf8", errors="ignore")
                        elif kind == 'associated_constant':
                            name_node = next((n for n, cname in node_captures if cname == "assoc_const_name"), None)
                            if name_node: name = byte_content[name_node.start_byte:name_node.end_byte].decode("utf8", errors="ignore")
                        else:
                        ...
                        if name == "<unknown>": continue # Skip if name extraction failed
                        ...
                        if decl_id not in declaration_map:
                            ...
                            if declaration_map[decl_id]['name'] == "<unknown>": declaration_map[decl_id]['name'] = full_name
        ...
        logger.debug(f"Tree-sitter Rust extracted {len(declarations)} declarations and {len(sorted_imports)} imports.")
