...
class EnhancedPythonParser(EnhancedBaseParser):
    ...
    def _process_block(self, lines: List[str], start: int, end: int, declarations: List[Declaration], 
                     imports: List[str], errors: List[str], parent_declaration: Optional[Declaration] = None):
        """
        ...
        logger.debug(f"Processing Python block at lines {start+1}-{end+1} (indent level: {current_indent})")
            ...
            for kind, pattern in self.patterns.items():
                ...
                if match:
                    ...
                    if ":" in line:
                        ...
                        expected_body_indent = current_line_indent + 4  # Python usually uses 4 spaces
                        ...
                        logger.debug(f"Found block for {kind} {name} from lines {start_line+1} to {end_line+1}")
                    ...
                    declaration = Declaration(
                        ...
                        modifiers=list(set(d for d in decorators if any(m in d for m in self.modifiers))),
                        children=[]
                    )
                    ...
                    if parent_declaration:
                        ...
                        logger.debug(f"Added nested declaration to parent {parent_declaration.name}: {kind} {name}")
                    else:
                ...
                if line.startswith('"""') or line.startswith("'''"):
                    ...
                    while k <= end and k < len(lines):
                        ...
                        if quote_type in current_line:
                            ...
                            docstring_lines.append(current_line[:idx])  # Add text before closing quotes
