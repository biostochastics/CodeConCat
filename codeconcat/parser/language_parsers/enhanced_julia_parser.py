...
class EnhancedJuliaParser(EnhancedBaseParser):
    ...
    def _setup_standard_patterns(self):
        ...
        self.modifiers = {
            "abstract", "baremodule", "begin", "const", "export",
            "global", "local", "macro", "mutable", "primitive",
            "quote", "using"
        }
    ...
    def _setup_julia_patterns(self):
        ...
        # Module pattern
        self.patterns["module"] = re.compile(
            r"^\s*(?:bare)?module\s+(?P<n>[\w.]+)",
            re.MULTILINE
        )
        
        # Type pattern
        self.patterns["type"] = re.compile(
            r"^\s*(?:abstract|primitive)\s+type\s+(?P<n>[\w]+)(?:\s*<:\s*[\w.]+)?",
            re.MULTILINE
        )
        ...
        self.patterns["inline_function"] = re.compile(
            r"^\s*(?P<n>[\w.!]+)\s*\([^)]*\)\s*(?:where\s+\{[^}]*\}\s*)?=(?![=>])",
            re.MULTILINE
        )
        
        # Import/using pattern
        self.patterns["import"] = re.compile(
            r"^\s*(?:import|using)\s+(.*?)(?::|$)",
            re.MULTILINE
        )
    ...
    def _process_block(self, lines: List[str], start: int, end: int, 
                     declarations: List[Declaration], imports: List[str],
                     errors: List[str], parent_decl: Optional[Declaration] = None) -> None:
        ...
        try:
                ...
                for kind, pattern in self.patterns.items():
                    ...
                    if match:
                        ...
                        if kind in ["module", "struct", "function", "macro", "type"]:
                            logger.debug(f"Found {kind} '{name}' at line {start_line}, finding block end...")
                        ...
                        if parent_decl is not None:
                            ...
                            logger.debug(f"Added nested declaration to parent {parent_decl.name}: {normalized_kind} {name}")
                        else:
            ...
            logger.debug(
                ...
                (f"{self._count_nested_declarations(declarations)} nested declarations" if declarations else "0 nested declarations")
            )
        except Exception as e:
    ...
    def _find_julia_block_end(self, lines: List[str], start: int) -> int:
        ...
        nesting_level = 1
        block_starters = [
            "module", "struct", "function", "macro", "type", 
            "begin", "for", "while", "if", "let", "try", "do"
        ]
        ...
        while j < len(lines):
            ...
            for word in block_starters:
                if re.search(fr'\b{word}\b', processed_line):
                    nesting_level += 1
                    logger.debug(f"Found nested block start at line {j+1}, nesting level: {nesting_level}")
