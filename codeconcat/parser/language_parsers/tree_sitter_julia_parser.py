...
class TreeSitterJuliaParser(BaseTreeSitterParser):
    ...
    def _run_queries(self, root_node: Node, byte_content: bytes) -> tuple[List[Declaration], List[str]]:
                ...
                if query_name == "imports":
                    ...
                    for node, capture_name in captures:
                        ...
                        while decl_node.parent and decl_node.type not in [
                            'module_definition', 'baremodule_definition', 'function_definition',
                            'macro_definition', 'struct_definition', 'abstract_type_definition',
                            'primitive_type_definition', 'global_declaration', 'constant_declaration',
                        ...
                        if capture_name in ["module", "baremodule", "function", "macro", "struct", "abstract_type", "primitive_type", "global_variable", "constant"]:
                    ...
                    for decl_id, data in node_capture_map.items():
                        ...
                        kind_map = {
                            'module_definition': 'module', 'baremodule_definition': 'module',
                            'function_definition': 'function', 'macro_definition': 'macro',
                            'struct_definition': 'struct', 'abstract_type_definition': 'abstract_type',
                            'primitive_type_definition': 'primitive_type',
                            'global_declaration': 'global_variable', 'constant_declaration': 'constant'
                        }
                        kind = kind_map.get(kind, kind)

                        if kind not in ['module', 'function', 'macro', 'struct', 'abstract_type', 'primitive_type', 'global_variable', 'constant']:
                            continue # Skip unclassified nodes

                        name_node = next((n for n, cname in node_captures if cname == "name"), None)
                        name = byte_content[name_node.start_byte:name_node.end_byte].decode("utf8", errors="ignore") if name_node else "<unknown>"

                        if name == "<unknown>": continue # Skip anonymous/unnamed
        ...
        logger.debug(f"Tree-sitter Julia extracted {len(declarations)} declarations and {len(sorted_imports)} imports.")
