    ...
    def parse(self, content: str, file_path: str) -> ParseResult:
        ...
        try:
            ...
            for i, line in enumerate(lines):
                ...
                bracket_level -= line.count('}')
                if bracket_level < 0: bracket_level = 0 # Avoid going negative
                ...
                if in_block_comment:
                    if "*/" in stripped_line:
                        ...
                        if in_doc_comment:
                            ...
                            if end_match:
                                doc_buffer.append(DOC_COMMENT_LINE_PATTERN.sub('\1', end_match.group(1)).strip())
                                in_doc_comment = False
                            else:
                                doc_buffer.append(DOC_COMMENT_LINE_PATTERN.sub('\1', line.strip()).strip())
                ...
                if "/*" in stripped_line:
                    ...
                    if doc_start_match:
                        ...
                        if "*/" in comment_content:
                            ...
                            doc_buffer = [DOC_COMMENT_LINE_PATTERN.sub('\1', end_match.group(1)).strip()] if end_match else []
                        else:
                            doc_buffer = [DOC_COMMENT_LINE_PATTERN.sub('\1', comment_content).strip()] # Start buffer
                        continue
                    else:
                ...
                if namespace_match:
                    ...
                    if name: # Handle anonymous namespaces later if needed
                        ...
                        declarations.append(Declaration(kind="namespace", name=full_name, start_line=i, end_line=i, docstring=docstring, modifiers=set())) # Placeholder end_line
                        doc_buffer = []
                    continue
                ...
                if csu_match:
                    ...
                    if name: # Skip anonymous structs/unions/enums for now
                        ...
                        docstring = "\n".join(filter(None, doc_buffer))
                        declarations.append(Declaration(kind=kind.replace(" ", "_"), name=full_name, start_line=i, end_line=i, docstring=docstring, modifiers=set())) # Placeholder end_line
                        doc_buffer = []
                ...
                if func_match:
                    ...
                    if name.startswith("~"): kind = "destructor"
                    elif current_class_stack and name == current_class_stack[-1]: kind = "constructor"
                    elif class_scope and name == class_scope.split('<')[0]: kind = "constructor" # Basic template check

                    full_name = "::".join(scope + [name])
                    docstring = "\n".join(filter(None, doc_buffer))
                    declarations.append(Declaration(kind=kind, name=full_name, start_line=i, end_line=i, docstring=docstring, modifiers=set())) # Placeholder end_line
                    doc_buffer = []
                    continue
                ...
                if typedef_match:
                    ...
                    full_name = "::".join(scope + [name])
                    docstring = "\n".join(filter(None, doc_buffer))
                    declarations.append(Declaration(kind=kind, name=full_name, start_line=i, end_line=i, docstring=docstring, modifiers=set()))
                    doc_buffer = []
                    continue
                ...
                if var_match:
                     ...
                     if not (stripped_line.endswith('{') or '(' in name): # Basic checks
                        ...
                        if current_class_stack: kind = "member_variable"
                        elif not current_namespace_stack and not current_class_stack: kind = "global_variable"
                        else: kind = "namespace_variable"
                ...
                if stripped_line:
                    ...
                    if stripped_line.startswith('}'):
                         if current_class_stack and bracket_level < len(current_namespace_stack) + len(current_class_stack) -1: # Approximation
                              current_class_stack.pop()
                         elif current_namespace_stack and bracket_level < len(current_namespace_stack) -1:
