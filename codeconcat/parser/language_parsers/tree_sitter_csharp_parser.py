...
def _clean_csharp_doc_comment(xml_content: str) -> str:
    ...
    try:
        ...
        cleaned_content = re.sub(r'<([a-zA-Z_][a-zA-Z0-9_]*)>', r'&lt;\1&gt;', cleaned_content) # Escape simple generics
        # Attempt to wrap
        ...
        for elem in root.iter():
            if elem.tag == 'param' and 'name' in elem.attrib:
                text_parts.append(f"@param {elem.attrib['name']}: {elem.text.strip() if elem.text else ''}")
            elif elem.tag == 'typeparam' and 'name' in elem.attrib:
                text_parts.append(f"@typeparam {elem.attrib['name']}: {elem.text.strip() if elem.text else ''}")
            ...
            elif elem.tag not in ['root', 'param', 'typeparam', 'returns', 'summary', 'see', 'exception'] and elem.text:
                ...
                if query_name == "imports":
                    for node, capture_name in captures:
                        ...
                        if '@' in capture_name: # Heuristic for alias parts
                            continue
                        if capture_name in ["import_path", "static_import_path", "aliased_namespace", "global_import_path"]:
                    ...
                    for node, capture_name in captures:
                        ...
                        if capture_name in ["namespace", "file_namespace", "class", "record", "record_struct", "interface", "struct", "enum", "method", "constructor", "destructor", "property", "indexer", "field", "constant", "delegate", "event", "event_accessor", "operator", "conversion_operator", "top_level_statement"]:
                            ...
                            current_decl_info['end_line'] = max(current_decl_info['end_line'], node.end_point[0])
