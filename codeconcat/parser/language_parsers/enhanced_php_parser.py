...
class EnhancedPHPParser(EnhancedBaseParser):
    ...
    def _setup_standard_patterns(self):
        ...
        self.modifiers = {
            ...
            "require", "require_once", "include", "include_once", "const",
            "global", "var", "readonly", "fn", "async"
        }
    ...
    def _setup_php_patterns(self):
        ...
        self.patterns["interface"] = re.compile(
            r"^\s*interface\s+(?P<name>\w+)(?:\s+extends\s+[\w,\s]+)?\s*\{",
            re.MULTILINE
        )
        
        # Trait declarations
        self.patterns["trait"] = re.compile(
            r"^\s*trait\s+(?P<name>\w+)\s*\{",
            re.MULTILINE
        )
        ...
        self.patterns["const"] = re.compile(
            r"^\s*(?:const|define\s*\(\s*[\'\"](?P<name>\w+)[\'\"]\s*,)",
            re.MULTILINE
        )
        ...
        self.patterns["use"] = re.compile(
            r"^\s*use\s+(?P<path>[\w\\]+)(?:\s+as\s+(?P<alias>\w+))?",
            re.MULTILINE
        )
                ...
                if match:
                    ...
                    if kind in block_types:
                        ...
                        if "{" in line:
                            ...
                            if not has_block and ("abstract" in line or "interface" in line) and ";" in line:
                                end_line = i
                    else:
                    ...
                    if normalized_kind in ["class", "interface", "trait", "function"] and end_line > start_line:
