...
from .file_parser import get_language_parser, determine_language
    ...
    for file_data in progress_iterator:
        ...
        try:
            ...
            if config.include_languages and language not in config.include_languages:
                logger.debug(
                    f"Skipping {file_path} - language '{language}' not in include list"
                )
                continue
                
            if language in config.exclude_languages:
                logger.debug(
                    f"Skipping {file_path} - language '{language}' in exclude list"
                )
                continue
            ...
            if not config.disable_tree:
                ...
                try:
                    tree_sitter_parser = get_language_parser(language, config, parser_type="tree_sitter")
                    if tree_sitter_parser:
                        ...
                        if isinstance(tree_sitter_parser, EnhancedParserInterface):
                            ...
                            if not tree_sitter_parser.validate():
                                logger.warning(f"Tree-sitter parser for {language} failed validation")
                                raise ValueError(f"Tree-sitter parser for {language} failed validation")
                    ...
                    if enhanced_parser:
                        ...
                        if isinstance(enhanced_parser, EnhancedParserInterface):
                            ...
                            if not enhanced_parser.validate():
                                ...
                                raise ValueError(f"Enhanced parser for {language} failed validation")
