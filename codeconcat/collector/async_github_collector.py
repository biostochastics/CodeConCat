"""Async Remote Git repository collector for CodeConcat."""
...
async def collect_git_repo_async(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        try:
            ...
            logger.info(f"Collecting files from temporary directory: {safe_temp_dir}")
            ...
            files = await loop.run_in_executor(
                None, collect_local_files, str(safe_temp_dir), config
            )
            logger.info(f"Found {len(files)} files in cloned repository.")
            return files, str(safe_temp_dir)  # Return files and temp_dir path
...
def build_git_clone_url(
    ...
    if token:
        ...
        safe_token = SecurityProcessor.sanitize_command_arg(token)
        # Insert token into URL for authentication
        return base_url.replace("https://", f"https://{safe_token}@")
    return base_url


# Create async wrapper for synchronous collect_git_repo function
async def collect_git_repo(
    source_url_in: str, config: CodeConCatConfig
) -> Tuple[List[ParsedFileData], str]:
    """
    Backward-compatible async wrapper for collect_git_repo.

    This function can be called from both sync and async contexts.
    """
    return await collect_git_repo_async(source_url_in, config)
