...
def verify_tree_sitter_dependencies() -> Tuple[bool, List[str], List[str]]:
    ...
    try:
        import tree_sitter
        logger.info(f"✅ tree-sitter core library found (version: {getattr(tree_sitter, '__version__', 'unknown')})")
    except ImportError as e:
        logger.error(f"❌ tree-sitter core library not found: {e}")
        return False, [], ["tree-sitter core library not found. Please install with 'pip install tree-sitter'."]
    ...
    if not hasattr(tree_sitter, "Language"):
        ...
        return False, [], ["tree-sitter is installed but appears to be incomplete. Try reinstalling with 'pip install -U tree-sitter'."]
    ...
    for language, parser_class_name in TREE_SITTER_PARSER_MAP.items():
        try:
            ...
            if hasattr(parser_instance, 'parser') and parser_instance.parser:
                ...
                error_msg = f"{language}: Parser instantiated but parser attribute is None or missing"
    ...
    for parser_type in parser_types:
        try:
            ...
            if parser_instance:
                ...
                if hasattr(parser_instance, "get_capabilities") and callable(parser_instance.get_capabilities):
                    try:
                        capabilities = parser_instance.get_capabilities()
                        results["parsers_found"][f"{parser_type}_capabilities"] = capabilities
                    except Exception as e:
                        results["errors"].append(f"Error getting capabilities for {parser_name}: {e}")
