...
class MarkdownRenderAdapter:
    ...
    def render_annotated_file(file_data: AnnotatedFileData, config: CodeConCatConfig) -> List[str]:
        ...
        if config.include_security_in_summary and file_data.security_issues and not config.mask_output_content:
            security_md = MarkdownRenderAdapter.render_security_issues(file_data.security_issues)
            if security_md:
                result.append(security_md + "\n")
                
        if config.include_tokens_in_summary and file_data.token_stats and config.enable_token_counting:
            tokens_md = MarkdownRenderAdapter.render_token_stats(file_data.token_stats)
            if tokens_md:
                result.append(tokens_md + "\n")
                
        # Add the file content
        content_to_render = file_data.annotated_content if file_data.annotated_content else file_data.content
...
class JsonRenderAdapter:
    ...
    def declaration_to_dict(decl: Declaration) -> Dict[str, Any]:
        ...
        return {
            ...
            "children": [
                JsonRenderAdapter.declaration_to_dict(child) for child in decl.children
            ],
        }
    ...
    @staticmethod
    def annotated_file_to_dict(file_data: AnnotatedFileData, config: CodeConCatConfig) -> Dict[str, Any]:
        ...
        if file_data.security_issues and not config.mask_output_content:
            result["security_issues"] = [
                JsonRenderAdapter.security_issue_to_dict(issue) for issue in file_data.security_issues
            ]
...
class XmlRenderAdapter:
    ...
    @staticmethod
    def create_annotated_file_element(file_data: AnnotatedFileData, config: CodeConCatConfig) -> ET.Element:
...
class TextRenderAdapter:
    ...
    def render_security_issues(issues: List[SecurityIssue]) -> List[str]:
        ...
        for issue in sorted_issues:
            result.append(f"[{issue.severity.value}] {issue.rule_id} - Line {issue.line_number}: {issue.description}")
